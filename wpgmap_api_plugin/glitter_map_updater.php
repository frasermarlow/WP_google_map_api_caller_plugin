...
Description:  Pulls data from ActiveCampaign and reformats it as a .json file that the WP Google Map plugin can ingest.  This version created April 2022.
Version:      1.2
...
function cronstarter_activation() {
	if( !wp_next_scheduled( 'map_cron_job' ) ) {  
	   wp_schedule_event( time(), get_option('glitter_map_update_frequency'), 'map_cron_job' );  // Valid values for the recurrence are ‘hourly’, ‘daily’, and ‘twicedaily’. Change to 'everyminute' for testing  
	}
}
...
function cronstarter_deactivate() {	
	...
	$timestamp = wp_next_scheduled ('map_cron_job');
	// unschedule previous event if any
	wp_unschedule_event ($timestamp, 'map_cron_job');
} 
...
add_action ('map_cron_job', 'glitter_update_map_json'); 
